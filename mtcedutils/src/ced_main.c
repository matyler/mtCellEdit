	...
	)
{
	...
	{
		int const c = (a + b) / 2;
		int const cmp = strcmp ( command, comtab[c].name );
...
static int get_filetype ( void )
{
	...
	{
		int const c = (a + b) / 2;
		int const cmp = strcmp ( global.s_arg, ftypes[c].name );
...
{
	set_global_sheet ( NULL );
}

void set_global_sheet (
	...
	)
{
	...
	{ "-help",	MTKIT_ARG_SWITCH, &show_version, 1, NULL },
	{ "-version",	MTKIT_ARG_SWITCH, &show_version, 2, NULL },
