...
{
	static char const	* dict_filename = NULL;
	static int		dict_range[4];

	if ( ! dict_filename )
	{
		dict_filename = global.s_arg;
		memcpy ( dict_range, global.i_range, sizeof(dict_range) );

		return 0;
	}

	// NOTE: inefficient to keep creating fuzz_dict for multiple input files
	FuzzFile * fuzz_dict = fuzz_file_new ( dict_filename, dict_range );
	FuzzFile * fuzz_in = fuzz_file_new ( global.s_arg, global.i_range );
	int const res = fuzz_file_match ( fuzz_dict, fuzz_in );

	if ( 0 == res )
	{
		set_global_sheet ( fuzz_file_steal_sheet ( fuzz_in ) );
	}
...
{
	FuzzFile	* const fuzzfile = user_data;
	...
	)
{
	...
	FuzzFile * fuzzfile = calloc ( 1, sizeof(FuzzFile) );
	...
	)
{
	...
	FuzzItem * item = calloc ( 1, sizeof(FuzzItem) );
	if ( ! item )
	{
		return 1;
	}
	...
	)
{
	...
	FuzzWord * word = calloc ( 1, sizeof(FuzzWord) );
	if ( ! word )
	{
		return 1;
	}
