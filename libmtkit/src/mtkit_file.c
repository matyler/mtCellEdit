	...
	)
{
	...
	buf = calloc ( newsize + (size_t)zero, 1 );
	if ( ! buf )
	{
		return NULL;
	}
	...
	)
{
	...
	do
	{
		...
		{
			bufsize += BUFSIZE;
			cp = realloc ( line, (size_t)bufsize );
	...
	)
{
	...
	while ( 1 )
	{
		...
		dest = realloc ( buf[0], buflen[0] + bufsize );
		if ( ! dest )
		{
			free ( buf );
			return 1;
...
mtFile * mtkit_file_open_mem ( void )
{
	...
	mtfp->buf_start = malloc ( MTKIT_FILE_MEM_BUFLEN );
	if ( ! mtfp->buf_start )
	{
		free ( mtfp );
	...
	)
{
	...
	else
	{
		if ( mem_len > mtfp->buf_left )
		{
			...
			np = realloc ( mtfp->buf_start, len + (size_t)todo );
			if ( ! np )
			{
				return 1;
			}
