/*
	Copyright (C) 2007-2015 Mark Tyler
...
#	pragma GCC diagnostic pop
	...
	char * nb = realloc ( bufree, s );	// Remove any slack memory
	...
	)
{
	...
	args = calloc ( (size_t)(arg_tot + 1), sizeof ( char * ) );
	...
	q = 0;
	cpos = 0;
	args[0] = malloc ( (size_t)(input_end - input + 1) );
	...
	for ( src = input; src[0]; src ++ )
	{
		...
		else if ( isspace ( src[0] ) )
		{
			if ( ! q )
			{
				...
				args[arg] = malloc ( (size_t)(input_end -
					src + 1) );
				cpos = 0;
	...
	args[arg][cpos] = 0;
	args[arg] = realloc ( args[arg], (size_t)(cpos + 1) );
