	...
	)
{
	...
	outbuf[0] = calloc ( 1, ((size_t)out_size) + 1 );
	if ( ! outbuf[0] )
	{
	...
	)
{
	...
	finfo = calloc ( 1, sizeof ( mtZipFileInfo ) );
	if ( ! finfo )
	{
	...
	finfo->buf = calloc ( 1, (size_t)finfo->buf_size );
	...
	)
{
	...
	do
	{
		...
		// We add an extra byte at end for 0 NUL byte convenience
		zip.mem = calloc ( 1, ((size_t)size_comp) + 1 );
		if ( ! zip.mem )
		{
			res = MTKIT_ZIP_ERROR_MEM;
			break;
...
*/
		...
		int i = callback ( zip.afilename, zip.mem, zip.size,
