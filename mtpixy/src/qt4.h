...
///	MENUS
	...
	void press_file_export_colormap ();
...
/// ----------------------------------------------------------------------------
	...
	QAction		* act_file_export_colormap;
...
public:
	DialogClickImage ( mtPixy::Image * im, int &px, int &py );
};
...
public:
	DialogPaletteNew ( int type, int num);
...
public:
	explicit DialogPaletteSort ( int coltot );
...
public:
	DialogColorEdit( Mainwindow &mw, unsigned char idx, mtPixy::Color * col)
		:
		mainwindow	( mw ),
		m_idx		( idx ),
		m_col		( col )
