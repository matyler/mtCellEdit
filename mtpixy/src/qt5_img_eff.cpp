...
{
	mtPixmap * const pixmap = backend.file.get_pixmap ();
	if ( pixmap )
	{
		DialogTransColor ( *this, pixmap );
	}
}
...
{
	DialogGetInt	dialog ( this, 1, 100, 50, "Sharpen", "Strength", 1 );
...
{
	DialogGetInt	dialog ( this, 1, 100, 50, "Soften", "Strength", 1 );
...
void Mainwindow::press_effects_normalize ()
{
	operation_update ( backend.file.effect_normalize (), "Normalize",
		UPDATE_ALL_IMAGE );
}

void Mainwindow::press_effects_bacteria ()
{
	DialogGetInt	dialog ( this, 1, 100, 10, "Bacteria", "Strength", 1 );
