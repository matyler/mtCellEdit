...
int DialogImageNew::get_type ()
{
	...
	{
		return mtPixy::Image::TYPE_RGB;
	}
	else if ( m_rbut_indexed->isChecked () )
	{
		return mtPixy::Image::TYPE_INDEXED;
	}
	...
	mainwindow	( mw )
{
	...
	mtPixy::Image * ti = NULL;

	if ( im->get_canvas_bpp () == 3 )
	{
		ti = im->scale ( m_pw, m_ph, mtPixy::Image::SCALE_BLOCKY );
	}
	else
	{
		mtPixy::Image * ati = im->scale ( m_pw, m_ph,
			mtPixy::Image::SCALE_BLOCKY );

		if ( ati )
		{
			ti = ati->convert_to_rgb ();
			delete ati;
			ati = NULL;
		}
	}

	QPixmap * pmap = mtQEX::qpixmap_from_pixyimage ( ti );
	delete ti;
	ti = NULL;
