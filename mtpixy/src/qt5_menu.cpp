...
{
	bool	const	b = backend.clipboard.get_pixmap () ? true : false;
	...
	mtPixmap const * const pixmap = backend.file.get_pixmap ();


	if ( pixmap && pixy_pixmap_get_alpha (pixmap) )
	{
	...
	if ( pixmap && pixy_pixmap_get_bytes_per_pixel (pixmap) ==
		PIXY_PIXMAP_BPP_RGB )
	{
	...
	else if ( pixmap && pixy_pixmap_get_bytes_per_pixel (pixmap) ==
		PIXY_PIXMAP_BPP_INDEXED )
	{
...
void Mainwindow::create_menu ()
{
	...
	for ( int i = 0; i < CLIPBOARD_MENU_TOTAL; i++ )
	{
		...
		connect ( act_edit_load_clipboard[i], &QAction::triggered,
			[i,this]
			{
				press_edit_load_clipboard ( i + 1 );
			} );
	...
	for ( int i = 0; i < CLIPBOARD_MENU_TOTAL; i++ )
	{
		...
		connect ( act_edit_save_clipboard[i], &QAction::triggered,
			[i,this]
			{
				press_edit_save_clipboard ( i + 1 );
			} );
	...
	QEX_MENU ( effects_crt, "CRT ...", NULL, NULL )
	...
	effects_menu->addAction ( act_effects_crt );
	...
	QEX_MENU ( options_show_files, "Show Command Line Files", NULL, NULL )
	...
	options_menu->addSeparator ();
	options_menu->addAction ( act_options_show_files );
	options_menu->addSeparator ();
	...
	act_options_show_files->setCheckable ( true );
