...
{
	int	const	type = prefs.getInt ( PREFS_FILE_NEW_PALETTE_TYPE );
	int	const	num = prefs.getInt ( PREFS_FILE_NEW_PALETTE_NUM );
	DialogPaletteNew dialog ( type, num );


	if ( dialog.exec () == QDialog::Accepted )
	{
		// OK pressed
		prefs.set ( PREFS_FILE_NEW_PALETTE_TYPE, dialog.get_type () );
		prefs.set ( PREFS_FILE_NEW_PALETTE_NUM, dialog.get_num () );
	...
	)
{
	...
	if ( type == 0 )
	{
		m_rbut_uniform->setChecked ( true );
	}
	else
	{
		m_rbut_balanced->setChecked ( true );
	}
