...
public:
	...
	void update_canvas_grid_menu ();

	bool check_main_key_event ( QKeyEvent * ev );

	void set_canvas_focus () const;
...
private:
	...
	mtPixmap * get_screenshot ();

	int project_load ( char const * fn );
	int project_save ( char const * fn, int ft );
	int project_new ( mtPixmap * im = NULL );
	int copy_selection ();
...
/// ----------------------------------------------------------------------------
	...
	QAction		* act_options_show_files;
	...
	std::unique_ptr<keyPressEater> m_key_eater;
	std::unique_ptr<FileListKeyEater> m_file_key_eater;
...
public:
	explicit PaletteHolder ( Mainwindow &mw );
	~PaletteHolder ();
...
public:
	explicit PaletteView ( Mainwindow &mw );
	~PaletteView ();
...
public:
	DialogPan ( Mainwindow &, int, mtPixmap const *, QScrollArea * );
...
class DialogCRT : public QDialog
{
	Q_OBJECT

public:
	explicit DialogCRT ( Mainwindow * parent );

	int get_scale ();

private:
	QSpinBox	* m_sbox_scale;
};
...
public:
	DialogBrushSettings ( Mainwindow * parent, int &s, int &f );
...
public:
	explicit DialogImageIndexed ( Mainwindow &mw );
...
public:
	DialogImageNew ( Mainwindow * parent, int w, int h, int t, bool clip );
...
public:
	explicit DialogImageResize ( Mainwindow &mw );
...
public:
	explicit DialogImageScale ( Mainwindow &mw );
...
public:
	explicit DialogPasteText ( Mainwindow &mw );
...
class DialogPaletteNew : public QDialog
{
	Q_OBJECT

public:
	DialogPaletteNew ( Mainwindow * parent, int type, int num);
...
private:
	...
	QRadioButton	* m_rbut_uniform;
	QRadioButton	* m_rbut_balanced;
	QRadioButton	* m_rbut_file;
...
public:
	explicit DialogPaletteSort ( Mainwindow * parent, int coltot );
...
private:
	...
	mtPalette	m_opal;
	mtPalette	* const	m_palette_live;
	int		const	m_rgb;
...
class DialogColorEdit : public QColorDialog
{
	Q_OBJECT

public:
	DialogColorEdit( Mainwindow &mw, unsigned char idx, mtColor * col)
		:
		QColorDialog	( &mw ),
		mainwindow	( mw ),
		m_idx		( idx ),
...
public:
	explicit keyPressEater ( Mainwindow &mw );
...
class FileListKeyEater : public QObject
{
	Q_OBJECT

public:
	explicit FileListKeyEater ( Mainwindow &mw );
