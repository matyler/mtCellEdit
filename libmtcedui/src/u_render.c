...
{
	outSTATE	* const	state = user_data;
	int		r = 255,
			g = 255,
			b = 255;
...
///	RENDER BACKGROUND
	...
	if ( cell->prefs )
	{
		r = MTKIT_INT_2_R ( cell->prefs->color_background );
		g = MTKIT_INT_2_G ( cell->prefs->color_background );
		b = MTKIT_INT_2_B ( cell->prefs->color_background );
	}
...
///	RENDER BORDER
	...
	{
		r = MTKIT_INT_2_R ( cell->prefs->border_color );
		g = MTKIT_INT_2_G ( cell->prefs->border_color );
		b = MTKIT_INT_2_B ( cell->prefs->border_color );
...
///	RENDER FOREGROUND
	...
	if ( cell->prefs )
	{
		r = MTKIT_INT_2_R ( cell->prefs->color_foreground );
		g = MTKIT_INT_2_G ( cell->prefs->color_foreground );
		b = MTKIT_INT_2_B ( cell->prefs->color_foreground );
	...
	)
{
	...
	state->mren_state.col_w = calloc ( (size_t)state->cols, sizeof( int ) );
	state->colx = calloc ( (size_t)state->cols, sizeof ( double ) );
	state->mren_state.cell_active = calloc ( (size_t)state->cols,
	...
	)
{
	...
	case HEADFOOT_SHEET_NAME:
		...
		{
			return state->sheet->book_tnode->key;
		}
...
{
	cwidSTATE	* const	state = user_data;
	...
	)
{
	...
	state.ax = calloc ( (size_t)state.as, sizeof ( int ) );
	state.aw = calloc ( (size_t)state.as, sizeof ( int ) );
...
{
	mrenSTATE	* const	state = user_data;
	...
	color[0] = (unsigned char)MTKIT_INT_2_R(cell->prefs->color_background );
	color[1] = (unsigned char)MTKIT_INT_2_G(cell->prefs->color_background );
	color[2] = (unsigned char)MTKIT_INT_2_B(cell->prefs->color_background );
	...
	)
{
	...
	else
	{
		if ( cell->prefs )
		{
			color[0] = (unsigned char)MTKIT_INT_2_R (
					cell->prefs->color_foreground );

			color[1] = (unsigned char)MTKIT_INT_2_G (
					cell->prefs->color_foreground );

			color[2] = (unsigned char)MTKIT_INT_2_B (
					cell->prefs->color_foreground );
		}
	}

	cui_ren_mem_to_rgb ( state->mem, mx, 0,
		mtkit_image_get_width ( state->image ),
		mtkit_image_get_height ( state->image ),
		pos_x, pos_w,
	...
	)
{
		...
		)
	{
		...
		im_width = mtkit_image_get_width ( state->image );
...
// Create an mtImage for this cell text + set mx position based on justification
	...
	)
{
	...
	state->image = cui_font_render ( state->viewren->font, new_text,
		CUI_CELL_MAX_PIXELS, text_style, state->viewren->row_pad );
	if ( ! state->image )
	{
		goto finish;
	}

	state->mem = mtkit_image_get_alpha ( state->image );
	if ( ! state->mem )
	{
		mtkit_image_destroy ( state->image );

		goto finish;
	}

	w = mtkit_image_get_width ( state->image );
	...
	)
{
		...
		)
	{
		...
		{
			rgb[0] = (unsigned char)MTKIT_INT_2_R (
				cell->prefs->border_color );

			rgb[1] = (unsigned char)MTKIT_INT_2_G (
				cell->prefs->border_color );

			rgb[2] = (unsigned char)MTKIT_INT_2_B (
				cell->prefs->border_color );
		}
	...
	pos_w = mtkit_image_get_width ( state->image );
	...
	if ( pos_w != mtkit_image_get_width ( state->image ) )
	{
		prepare_ren_expansion ( state, pos_x, pos_w, row, col, cell,
			col );
	}
...
{
	mrenSTATE	state = { row_start, col_start, viewren, x, y, w, h,
	...
	state.rgb = malloc ( (size_t)(w * state.row_h * 3) );
	if ( ! state.rgb )
	{
		return 1;
	}
	...
	state.cell_active = malloc ( (size_t)(state.c2 - state.c1 + 1) );
	...
	)
{
	...
	for ( i = r1; i <= r2; i++ )
	{
		...
		image = cui_font_render ( viewren->font, txt, 0, 0,
			viewren->row_pad );
		if ( ! image )
		{
			continue;
		...
		mem = mtkit_image_get_alpha ( image );
		if ( ! mem )
		{
		...
		image_w = mtkit_image_get_width ( image );
		image_h = mtkit_image_get_height ( image );
	...
	)
{
	...
	for ( i = c1; i <= c2; i++ )
	{
		...
		image = cui_font_render ( viewren->font, txt, 0, 0, 0 );
		if ( ! image )
		{
			continue;
		}
