...
{
	int		i;
	double const	visible_width = state->pgv_xr - state->pgv_xl;


	for ( i = col; i < state->cols; i++ )
	{
		double const cw = state->colx[i] - state->colx[col] +
	...
	)
{
	...
	for ( int j = 0; j < yh; j++ )
	{
		unsigned char * src = mem + mw * ( j + my1 ) + mx1 + mxo;
