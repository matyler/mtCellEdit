	...
	)
{
	...
	atom = calloc ( 1, sizeof ( CuiUndoAtom ) );
	if ( ! atom )
	{
		return 1;
	}
...
{
	CuiUndoStep	* step = NULL;


	step = calloc ( 1, sizeof ( CuiUndoStep ) );
	if ( ! step )
	{
		return NULL;
	}
	...
	if ( sheet )
	{
		...
		step->sheet_name = strdup ( sheet->book_tnode->key );
		if ( ! step->sheet_name )
		{
			goto error;
		}
...
{
	CedCellRef	* const	cref = user_data;
...
CuiBook * cui_book_new ( void )
{
	...
	cubook = calloc ( sizeof ( CuiBook ), 1 );
	...
	)
{
	...
	if ( undo_data )
	{
		undo_data->atom->charp[0] = strdup ( shold->book_tnode->key );
		if ( ! undo_data->atom->charp[0] )
		{
			goto error_report;
		}
