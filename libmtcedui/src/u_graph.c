...
{
	scanSTATE	* const	state = user_data;
	...
	)
{
	...
	for ( i = 0; table_d[i].name != NULL; i++ )
	{
		...
		if ( mtkit_strtoddt ( tn->data, table_d[i].var ) == 0 )
		{
			continue;	// Parsed as a date/time
		}

		// Parse this text via libmtcelledit
		parser_state = ced_sheet_parse_text ( sheet, 1, 1, tn->data,
			NULL );
	...
	)
{
	...
	if ( tn )
	{
		state->sheet = ced_book_get_sheet ( state->book, tn->data );
	...
	)
{
	cairo_set_source_rgb ( state->cr,
		(double)MTKIT_INT_2_R ( rgb ) / 255.0,
		(double)MTKIT_INT_2_G ( rgb ) / 255.0,
		(double)MTKIT_INT_2_B ( rgb ) / 255.0
		);
}
...
{
	renSTATE	* const	state = user_data;
	int			i;
	...
	)
{
	...
	memset ( state, 0, sizeof ( renSTATE ) );
	...
	)
{
	...
	image = mtkit_image_from_cairo ( state.surface );
