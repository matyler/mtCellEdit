...
Mainwindow::Mainwindow ( Core & core )
	:
	m_window_width	( core.get_output_width () ),
	m_window_height	( core.get_output_height () ),
	m_core		( core ),
	m_animate	( *this, core ),
	m_gin		( SDL_INIT_VIDEO ),
	m_window	( GIN_WINDOW_CANVAS_2D )
{
	m_window.init ( "mtMandy - Mandelbrot set explorer",
	...
	m_mandy.set_verbose ( core.get_verbose() );
	m_mandy.set_depth_max ( core.get_depth_max() );

	if ( 0 == m_animate.init ( &m_mandy ) )
	{
		set_mode ( MODE_ANIMATION_RENDERING );
	}
	else
	{
		centre_frame ();
		...
		m_mandy.zoom_cxyrange ( core.get_axis_cx(), core.get_axis_cy(),
			core.get_axis_range() );
...
void Mainwindow::window_render ()
{
	if (	MODE_INTERACTIVE_RENDERING == m_mode
		|| MODE_ANIMATION_RENDERING == m_mode
		)
	{
		prepare_surface ();
	}
...
void Mainwindow::frame_cycle ()
{
	if ( MODE_INTERACTIVE_RENDERING == m_mode )
	{
		if ( Mandelbrot::STATUS_IDLE == m_mandy.get_status() )
		{
			set_mode ( MODE_INTERACTIVE_FRAMING );
		}
	}
	else if ( MODE_ANIMATION_RENDERING == m_mode )
	{
		if ( Mandelbrot::STATUS_IDLE == m_mandy.get_status() )
		{
			if ( m_anim_frame >= 0 )
			{
				if ( m_animate.save_frame ( m_anim_frame ) )
				{
					quit();
				}
			}
...
void Mainwindow::set_mode ( int const mode )
{
	switch ( mode )
	{
	case MODE_INTERACTIVE_RENDERING:
		m_gin.set_fps_max ( MODE_INTERACTIVE_RENDERING_FPS );
		break;

	case MODE_INTERACTIVE_FRAMING:
		m_gin.set_fps_max ( MODE_INTERACTIVE_FRAMING_FPS );
		break;

	case MODE_ANIMATION_RENDERING:
		m_gin.set_fps_max ( MODE_ANIMATION_RENDERING_FPS );
		break;
	}
...
void Mainwindow::start_rendering ( int const zoom )
{
	...
	m_mandy.build_mandelbrot_set ( m_core.get_thread_total () );
...
{
	double const range_w = m_mandy.get_range_w();
	double const cx = m_mandy.get_cx();
	double const cy = m_mandy.get_cy();

	printf ( "-cx %.15g -cy %.15g -range %.15g\n", cx, cy, range_w );
}
...
void Mainwindow::output_image_file () const
{
	...
	char buf[256];
	snprintf ( buf, sizeof(buf), BIN_NAME "_%.15g_%.15g_%.15g.png", cx, cy,
		range_w );
