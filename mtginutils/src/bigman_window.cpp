	...
	m_window	( GIN_WINDOW_CANVAS_2D )
{
...

	{
		...
		m_mandy.zoom_cxyrange ( core.get_axis_cx_r(),
			core.get_axis_cy_r(),
			core.get_axis_range_r() );
...
{
	std::cout
		<< "-cx " << m_mandy.get_cx_r().to_string()
		<< "-cy " << m_mandy.get_cy_r().to_string()
		<< "-range " << m_mandy.get_range_w_r().to_string()
		<< "\n";
}
...
void Mainwindow::output_image_file () const
{
	...
	filename += BIN_NAME;
	filename += "_";
	filename += m_mandy.get_cx_r().to_string();
	filename += "_";
	filename += m_mandy.get_cy_r().to_string();
	filename += "_";
	filename += m_mandy.get_range_w_r().to_string();
	filename += ".png";
