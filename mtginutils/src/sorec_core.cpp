	...
	)
{
	...
	args.add ( "dev",		m_dev_record );
	args.add ( "gui",		m_gui, 1 );
	args.add ( "ldev",		[]()
	{
		int const count = SDL_GetNumAudioDevices ( 1 );
	...
	{
		std::cerr << "Error - No ouput filename given!\n";
		return 1;		// Stop program
	}

	if ( 1 == mtkit_file_readable ( m_output_filename ) )
	{
...
int Core::audio_init ()
{
	...
	if (	audio_record.open_device ( get_dev_record (), spec )
		|| this->audio_file.open ( m_output_filename,
			audio_record.get_spec () )
		)
	{
		return 1;
	}

	audio_record.set_file ( &audio_file );
