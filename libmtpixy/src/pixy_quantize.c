/*
	...
	Copyright (C) 2009-2013,2016-2021 Mark Tyler
...
//	bins[0].bk = bins[i].fw = 0;
	...
	pixy_palette_set_size ( pal, MAX ( j, PIXY_PALETTE_COLOR_TOTAL_MIN ) );
...
{
	if (	! pixmap->canvas			||
		PIXY_PIXMAP_BPP_RGB != pixmap->bpp	||
		coltot < PIXY_PALETTE_COLOR_TOTAL_MIN	||
		coltot > PIXY_PALETTE_COLOR_TOTAL_MAX
		)
	{
		return 1;
	}
	...
	if ( pal )
	{
		if ( pnnquan ( pixmap->canvas, pixmap->width,
			pixmap->height, coltot, pal ) )
		{
			return 1;
		}

		pixy_palette_set_size ( pal, coltot );
	}
	else
	{
		if ( pnnquan ( pixmap->canvas, pixmap->width,
			pixmap->height, coltot, &pixmap->palette ) )
		{
			return 1;
		}
	}
