...
CedSheet * ced_sheet_new ( void )
{
	...
	sheet = calloc ( sizeof ( CedSheet ), 1 );
	if ( ! sheet )
	{
		return NULL;
	}
	...
	)
{
	...
	else
	{
		...
		cnode = mtkit_tree_node_find ( ctree,
	...
	)
{
	...
	{
		cell = ced_cell_set_find ( sheet, 0, column );
		if ( ! cell )
		{
			return -1;
		}
	...
	)
{
	...
	// Find the cell in the row
	cnode = mtkit_tree_node_find ( (mtTree *)rnode->data,
		(void *)(intptr_t)column );
	if ( ! cnode )
	{
...
{
	autocolSTATE	* const	state = user_data;
	char		* txt = NULL;
	int		ci;
	...
	)
{
	...
	width_list[0] = calloc ( (size_t)coltot, sizeof ( int ) );
	if ( ! width_list[0] )
	{
		return 1;
	}
	...
	)
{
	...
	tr1 = MIN ( sheet->prefs.cursor_r1, sheet->prefs.cursor_r2 );
	tr2 = MAX ( sheet->prefs.cursor_r1, sheet->prefs.cursor_r2 );
	tc1 = MIN ( sheet->prefs.cursor_c1, sheet->prefs.cursor_c2 );
	tc2 = MAX ( sheet->prefs.cursor_c1, sheet->prefs.cursor_c2 );
...
{
	return calloc ( sizeof ( CedSheetPrefs ), 1 );
}
