	...
	)
{
	...
	{
		mtTreeNode * const tn1 = mtkit_tree_node_find ( r1,
			(void *)(intptr_t)state.cols[i] );
		mtTreeNode * const tn2 = mtkit_tree_node_find ( r2,
			(void *)(intptr_t)state.cols[i] );
		...
		{
			CedCell const * const c1 = (CedCell const *)(tn1->data);
			CedCell const * const c2 = (CedCell const *)(tn2->data);
