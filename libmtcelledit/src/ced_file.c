...
static char const	* ledger_header = "ledger\t==!\"$CED\tTSV\n";
...
{
	static char const * txt[CED_FILE_TYPE_TOTAL] = {
		"?",
		"TSV content",
		"TSV.gz content",
...
{
	tsv_state		* const	state = user_data;
	...
	)
{
	...
	res = ced_sheet_scan_area ( sheet, 1, 1, 0, 0, tsv_save_cb, &state );
	if ( ! res )
	{
		// Add final newline
	...
	)
{
	...
	res = mtkit_file_write_string ( state.mtfp, ledger_header );
	if ( res == 0 )
	{
	...
	)
{
	...
	for ( i = 0; i < 4; i ++ )
	{
		bord = ( cell->prefs->border_type >> shft[i] ) &
...
{
	static char	const * const html_header =
	...
	static char	const * const html_footer =
			"	</TR>\n"
			"</TABLE>\n"
			"<P><BR><BR>\n"
			"</P>\n"
