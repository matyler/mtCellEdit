...
{
	return calloc ( sizeof ( CedCell ), 1 );
}
...
CedCellPrefs * ced_cell_prefs_new ( void )
{
	...
	prefs = calloc ( sizeof ( CedCellPrefs ), 1 );
	if ( ! prefs )
	{
		return NULL;
	}

	prefs->color_background = 16777215;
...
{
	nw->key = old->key;
	nw->data = ced_cell_duplicate ( old->data );
...
{
	nw->key = old->key;
	nw->data = mtkit_tree_duplicate ( old->data, col_dup_func );
	...
	)
{
	...
	{
		if ( state->callback ( state->sheet, (CedCell *)col_node->data,
