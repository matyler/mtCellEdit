...
{
	CedBookFile	* bookfile = node->data;
	...
	)
{
	...
	ced_book_destroy_sheet ( sheet->book, sheet->book_tnode->key );
	...
	)
{
	...
	bookfile = calloc ( sizeof ( CedBookFile ), 1 );
	if ( ! bookfile )
	{
		free ( key );
...
{
	cprefSTATE	* const	state = user_data;
...
{
	int			res = 1;
	bookSTATE	* const state = user_data;
	mtFile			* mtft;
	...
	)
{
	...
	filename = mtkit_string_join ( state->tmp_fname, "/", node->key, NULL );
	if ( ! filename )
	{
		return 1;
	}
	...
	)
{
	if ( name )
	{
		...
		case 0:	// LOAD SHEET
			...
			sheet = ced_sheet_load_mem ( buf, (size_t)buf_len,
		...
		default: // LOAD FILE
			...
			bookfile = ced_book_add_file ( state->book, buf,
				buf_len, s );
...
{
	mergeSTATE	* const	state = user_data;
	...
	)
{
	...
	{
		state.sheet_a = calloc ( (size_t)state.sheet_tot,
	...
	if ( state.sheet_a )
	{
		...
		for ( sp = 0; sp < state.sheet_i; sp++ )
		{
			...
			name = strdup ( sheet->book_tnode->key );
	...
	{
		state.file_a = calloc ( (size_t)state.file_tot,
