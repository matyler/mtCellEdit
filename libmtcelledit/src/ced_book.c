...
{
	CedBook * const book = (CedBook *) ( calloc ( 1, sizeof ( CedBook ) ) );
	...
	)
{
	...
	char * const key = strdup ( filename );
	...
	CedBookFile * const bookfile = (CedBookFile *)
		( calloc ( sizeof ( CedBookFile ), 1 ) );
	...
	)
{
	...
	mtTreeNode * const node = mtkit_tree_node_find( book->files, filename );
	if ( ! node )
	{
		return NULL;
	}
	...
	)
{
	...
	struct tm	* const now_tm = localtime ( &now );
	...
	)
{
	...
	mtFile * mtft = ced_sheet_save_mem (
		(CedSheet *)(node->data), state->filetype );
	...
	int res = save_mtft_to_zip ( mtft, state, (char *)node->key );
	...
	return sheet_prefs_utree ( state->book_prefs_node, (char *)node->key,
	...
	)
{
	...
	CedBookFile * const bookfile = (CedBookFile *)(node->data);
	char * const filename = mtkit_string_join ( state->tmp_fname, "/",
	...
	int const res = mtkit_zip_save_file ( state->zip, filename,
		bookfile->mem, bookfile->size, 1, bookfile->timestamp[0],
...
{
	mtUtreeNode * const node = mtkit_utree_get_node ( state->book_prefs,
		CED_FILE_PREFS_BOOK, MTKIT_UTREE_NODE_TYPE_ELEMENT );
	...
	for (	chn = mtkit_utree_get_node ( node,
			CED_FILE_PREFS_SHEET, MTKIT_UTREE_NODE_TYPE_ELEMENT );
		chn ;
		...
		)
	{
		...
		CedSheet * const sheet = ced_book_get_sheet( state->book, txt );
		if ( ! sheet )
		{
			continue;
		}
	...
	)
{
	...
	if ( state.sheet_a )
	{
		...
		{
			CedSheet * const sheet = state.sheet_a[ sp ];
			if ( ! sheet )
			{
				break;
			}
...
//				state.res = 2;
	...
	if ( state.file_a )
	{
		...
		for ( sp = 0; sp < state.file_i; sp++ )
		{
			...
			CedBookFile * const bookfile = (CedBookFile *)
				( state.file_a[ sp ]->data );

			char * const filename = (char *)state.file_a[ sp ]->key;
