...
void Mainwindow::create_main_ui ()
{
	...
	m_combo = new fileCombo ( uprefs, "Output Directory",
		QFileDialog::Directory );

	layv->addWidget ( m_combo );

	m_combo->repopulate ( mprefs.recent_dir );
	...
	m_input = new groupList ( uprefs, "Input Files" );
	split->addWidget ( m_input );

	m_bottles = new groupList ( uprefs, "Input Bottles" );
...
int Mainwindow::validate_output_dir ()
{
	...
	backend.mprefs.recent_dir.set ( dir );
	m_combo->repopulate ( mprefs.recent_dir );
