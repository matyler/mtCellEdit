...
/// ----------------------------------------------------------------------------
	...
	m_otp_list->verticalHeader ()->setSectionResizeMode(QHeaderView::Fixed);
...
void DialogButtInfo::press_bucket_add ()
{
	...
	mtQEX::BusyDialog busy ( this, nullptr,
		[&error, &busy, &db, &total]()
		{
			...
			mtKit::Busy * const m_busy = busy.get_busy ();

			m_busy->set_minmax ( 0, total );
