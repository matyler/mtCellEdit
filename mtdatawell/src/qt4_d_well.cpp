	...
	mainwindow	( mw )
{
	...
	button = new QPushButton ( QIcon::fromTheme("view-refresh"),
		"Reset Seed" );
	hbox->addWidget ( button, 1, BUTTON_COL );
	connect ( button, SIGNAL ( clicked () ), this,
		SLOT ( press_seed_reset () ) );

	button = new QPushButton ( QIcon::fromTheme("view-refresh"),
		"Reset Shifts" );
	hbox->addWidget ( button, 2, BUTTON_COL );
	connect ( button, SIGNAL ( clicked () ), this,
...
{
	mtKit::Prefs &prefs = mainwindow.prefs;
	...
	if ( dialog.exec () )
	{
		...
		prefs.set ( PREFS_LAST_DIRECTORY, dialog.directory().
