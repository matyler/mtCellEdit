...
#define APP_TITLE_TXT "Coin Toss"



void Mainwindow::press_apps_coins ()
{
	DialogCoinToss dialog ( *this );
}



DialogCoinToss::DialogCoinToss (
	Mainwindow	&mw
	)
	:
	QDialog		( &mw ),
	...
	m_well		( mw.backend.db.get_well () )
{
	...
	m_total->setRange ( mtDW::Well::COIN_TOTAL_MIN,
		mtDW::Well::COIN_TOTAL_MAX );
	m_total->setValue ( mtDW::Well::COIN_TOTAL_DEFAULT );
...
void DialogCoinToss::press_apply ()
{
	int const total = m_total->value ();
	std::string txt;

	m_well->app_coin_toss ( txt, total );
