	...
	m_sheet		( sheet )
{
	...
	QLineEdit * const edit = new QLineEdit ( mtQEX::qstringFromC ( path ) );
	...
	)
{
	...
	for ( int r = 1; r <= m_row_total; r++ )
	{
		for ( int c = 1; c < RAFT_COL_TOTAL; c++ )
		{
			...
			char * csp = ced_cell_create_output ( cell, NULL );
			if ( ! csp )
			{
				continue;
			}
...
{
	char * new_path = raft_path_merge ( m_path, m_sheet, row + 1 );
	if ( new_path )
	{
		mainwindow.analyse ( new_path );
		free ( new_path );
		new_path = NULL;
	}
}
