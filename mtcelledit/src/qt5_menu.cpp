...
QEX_MENU ( FileQuit, "Quit", "Ctrl+Q", "application-exit" )
	...
	for ( int i = 0; i < RECENT_MENU_TOTAL; i++ )
	{
		...
		connect ( act_FileRecent[i], &QAction::triggered, [i,this]
			{
				press_FileRecent ( i + 1 );
			} );
...
QEX_MENU ( EditSelectAll, "Select All", "Ctrl+A", "edit-select-all" )
	...
	act_EditUseSystemClipboard->setChecked ( mprefs.clipboard_use_system ==
		0 ? false : true );
...
typedef struct
{
	...
	for ( int i = 0; i < BORDER_MENU_TOTAL; i++ )
	{
		...
		connect ( act_OptionsBorder[i], &QAction::triggered, [i,this,bp]
			{
				press_OptionsBorder ( bp[i].num );
			} );
	}
...
#define MENTS( A, B, C, D, E ) \
	...
	connect ( act_OptionsTextStyle_ ## A, &QAction::triggered, [this] \
		{ press_OptionsTextStyle ( E ); } );
...
QEX_MENU ( FindSheets, "All Sheets", NULL, NULL )
	...
	act_FindWildcards->setChecked ( mprefs.find_wildcards == 0 ?
		false : true );

	act_FindCase->setCheckable ( true );
	act_FindCase->setChecked ( mprefs.find_case_sensitive == 0 ?
		false : true );

	act_FindValue->setCheckable ( true );
	act_FindValue->setChecked ( mprefs.find_value == 0 ? false : true );

	act_FindSheets->setCheckable ( true );
	act_FindSheets->setChecked( mprefs.find_all_sheets == 0 ? false : true);
