...
void CedView::setPanes ()
{
	...
	else
	{
		...
		{
			int const w = cui_ren_x_from_column ( min_col, crender,
				max_col + 1 );
...
{
	CedSheet	* const sheet = mainwindow->projectSetSheet ();
	...
	)
{
	...
	CuiRender	* const ren = mainwindow->projectGetRender ();
	...
	)
{
	...
	if ( row >= vscroll->minimum () )
	{
		...
		{
			// Last row visible
			int last_row = cui_ren_row_from_y( vscroll_val, crender,
	...
	if ( column >= hscroll->minimum () )
	{
		...
		{
			// Last row visible
			int last_col = cui_ren_column_from_x ( hscroll_val,
				crender, area [ CEDVIEW_AREA_BR ]->width () );
