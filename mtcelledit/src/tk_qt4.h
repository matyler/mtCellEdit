...
public:
	...
	void		setCursorRange (
				...
				int		force_update,
				int		follow_all
					// Force cursor visibility everywhere
				);

	void		setChangesFlag ( void );
...
public:
	...
	int		getHScrollValue ();
	int		getVScrollValue ();
	int		getVisibleRows ( CuiRender * render = NULL );
...
protected:
	bool		event ( QEvent * event );	// Catch tabs
...
public:
	SortDialog (	int		axis,	 // 0 = Rows, 1 = Columns
			QWidget		* parent = NULL
			);
...
public:
	explicit SwatchDialog ( QString title );
