	...
	)
{
	...
	{
	case Qt::Key_Tab:
		CedSheet * const sheet = cedview->mainwindow()->
			projectGetSheet ();
		...
		int r = sheet->prefs.cursor_r2;
		int c = sheet->prefs.cursor_c2;
		...
		cedview->mainwindow()->setCursorRange ( r, c, r, c, 1, 0, 0 );
...
{
	int const wheel_delta = (ev->angleDelta() / 8).y();
	...
	)
{
	...
	sheet = cedview->mainwindow()->projectGetSheet ();
	if ( ! sheet )
	{
		return;
	}
	...
	if ( 2 == caller && cedview->mainwindow()->isEditingCellText () )
	{
		return;
	}

	crender = cedview->mainwindow()->projectGetRender ();
	...
	case 0:	// Mouse press on sheet area
		...
		if ( cedview->mainwindow()->isEditingCellText () )
		{
			...
			cedview->mainwindow()->insertCellText ( txt );
		...
		{
			cedview->mainwindow()->setCursorRange ( r, c, r, c, 1,
				0, 0 );
		}
		else if ( 3 == b )	// Right click - set second corner
		{
			cedview->mainwindow()->setCursorRange (
	...
	case 1:	// Mouse press on header area
		...
		cedview->mainwindow()->setCursorRange( r1, c1, r2, c2, 0, 0, 0);
		break;
